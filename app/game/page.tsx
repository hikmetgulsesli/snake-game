...
export default function GameArena({ initialDifficulty = 'NORMAL' }: GameArenaProps = {}) {
  ...
  }, [state.status, changeDirection, handlePauseToggle]);
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] flex flex-col">
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-4 max-w-4xl mx-auto w-full">
        {/* HUD - Score, Speed, High Score, Pause Button */}
        <HUD
          score={state.score}
          speed={state.speed}
          ...
          showPauseButton={state.status === 'PLAYING' || state.status === 'PAUSED'}
        ...
        <div className="relative">
          ...
          {/* Pause Overlay */}
          {state.status === 'PAUSED' && (
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/80 rounded-xl" data-testid="pause-overlay">
              ...
              <p className="mt-4 text-gray-400 text-sm">Press P to resume</p>
            </div>
          )}
        ...
        <div className="mt-8 flex flex-col sm:flex-row items-center gap-4 bg-[#1e1e1e] rounded-xl p-4 border border-[#2a2a2a]">
          ...
          <div className="flex items-center gap-2">
            <kbd className="px-3 py-1 bg-[#2a2a2a] border border-gray-600 rounded text-xs text-gray-300">P</kbd>
