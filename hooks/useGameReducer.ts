...
import { DIFFICULTY_SETTINGS, OPPOSITE_DIRECTIONS, BASE_SCORE_PER_FOOD } from '../constants/game';
...
export function calculateScore(
  ...
  return Math.floor(foodsEaten * BASE_SCORE_PER_FOOD * multiplier);
}

/**
 * Calculates new speed after eating food with minimum threshold
 */
export function calculateNewSpeed(
  currentSpeed: number,
  speedDecrement: number,
  minSpeed: number
): number {
  return Math.max(minSpeed, currentSpeed - speedDecrement);
}
  ...
  switch (action.type) {
    ...
    case 'MOVE_SNAKE': {
      ...
      if (isFoodEaten(newHead, state.food)) {
        ...
        const newFoodsEaten = state.foodsEaten + 1;
        
        // Calculate new score with difficulty multiplier
        const newScore = calculateScore(
          newFoodsEaten,
          state.config.scoreMultiplier
        );
        
        // Calculate new speed with minimum threshold
        const newSpeed = calculateNewSpeed(
          state.speed,
          state.config.speedIncrement,
